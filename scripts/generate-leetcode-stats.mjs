...
const response = await fetch(endpoint, {
  ...
  headers: {
    ...
    "user-agent": "Mozilla/5.0 moinaktar-profile"
  },
  body: JSON.stringify({
...
const solved = Object.fromEntries(
  user.submitStatsGlobal.acSubmissionNum.map(x => [x.difficulty, x.count])
);
...
const bar = (label, value, y, fill) => `
  ...
  <rect x="66" y="${y + 12}" width="${Math.max(8, Math.round(628 * value / max))}" height="8" rx="4" fill="${fill}"/>
`;

const topicWidths = topics.map(t => Math.max(88, 24 + t.tagName.length * 7));
let tx = 804;
...
<defs>
  ...
  <style>
    ...
    .topic{font:600 11px Arial,sans-serif;fill:#D9D4C7;letter-spacing:1px}
  </style>
</defs>
...
<rect class="panel" x="42" y="86" width="264" height="92" rx="6"/>
<text x="64" y="113" class="label">TOTAL SOLVED</text><text x="64" y="151" class="num">${total}</text>
<rect class="panel" x="322" y="86" width="264" height="92" rx="6"/>
<text x="344" y="113" class="label">EASY</text><text x="344" y="151" class="num">${easy}</text>
<rect class="panel" x="602" y="86" width="264" height="92" rx="6"/>
<text x="624" y="113" class="label">MEDIUM</text><text x="624" y="151" class="num">${medium}</text>
<rect class="panel" x="882" y="86" width="276" height="92" rx="6"/>
<text x="904" y="113" class="label">HARD</text><text x="904" y="151" class="num">${hard}</text>
<rect class="panel" x="42" y="198" width="716" height="188" rx="6"/>
<text x="66" y="228" class="label">DIFFICULTY DISTRIBUTION</text>
${bar("EASY", easy, 266, "#B8A06A")}
${bar("MEDIUM", medium, 314, "#C4BCA8")}
${bar("HARD", hard, 362, "#6F5B38")}
<rect class="panel" x="780" y="198" width="378" height="188" rx="6"/>
<text x="804" y="228" class="label">CURRENT STREAK</text>
